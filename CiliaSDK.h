...
namespace CiliaSDK
{
    class CiliaSDK
    {
        ...
        {  
            ...
            int32_t port = 1995;//port we will be communicating on
            ...
            /*String to hold messages received over network from games or control panel*/
            std::string data = null;
            /*Used to hold tokenized versions of messages in data*/
            std::string dataSplit[];
            /*Used to hold further tokenized versions of messages*/
            std::string dataSplit2[];
            /*
            ...
            while (!mShutdown)
            {
                try
                {
                    ...
                    unsigned char[] bytes = new Byte[10000];
                    ...
                    while (!mShutdown)
                    {
                        ...
                        while ((i = mStream.Read(bytes, 0, bytes.Length)) != 0)
                        {
                            ...
                            for (std::string subString : dataSplit)
                            {
                        ...
                        {
                            std::cout << "ObjectDisposedException" << std::endl;
                        }
        ...
        void SpecialProcess(string processString)
        {
            ...
            else if (processString.Contains("SetLights"))
            {
                ...
                tSplit = processString.Split('|');
                int tCiliaNumber = int.Parse(tSplit[1]);
                std::string tLightValue1 = tSplit[2];
                std::string tLightValue2 = tSplit[3];
                std::string tLightValue3 = tSplit[4];
                std::string tLightValue4 = tSplit[5];
                std::string tLightValue5 = tSplit[6];
                std::string tLightValue6 = tSplit[7];
                ...
                std::string tCOMNAME = "COMPORT" + tCiliaNumber.ToString();
                System.IO.File.WriteAllText(Path.Combine(mProfileUtil.GetGameDirectory(), tCOMNAME), mCiliaManager.GetCiliaToStringForWritingToFile(tCiliaNumber));
            }
        ...
        {
            std::string processString = processStringObj.ToString();
            std::string processStringSplit[];
            try
            {
         ...
         */
        void SubProcessMessage(int position, std::string processStringSplit[], std::string processString2)
        {
         ...
         */
        void RawSubProcessMessage(int position, std::string processString2)
        {
            int tempCiliaNumber = 0;
            for (int ij = 0; ij < Cilia.GetCountOfGroup((byte)position); ij++)
            {
